	...
	render() {
		switch (this.state.type) {
			...
			case "no-comments-enabled":
				...
				`;
				// Local patch: when hide="auto", stay silent for posts that don't
				// have a linked Bluesky post yet (no bskyPostRef on the document)
				// instead of rendering a "not enabled" notice on every old post.
				if (this.hide) {
					this.commentsContainer.style.display = "none";
				}
				break;
