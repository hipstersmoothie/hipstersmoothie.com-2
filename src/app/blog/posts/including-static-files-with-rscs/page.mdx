...
Now lets say you want to build and Server Component that uses a static file.
When this is deployed is isn't deployed, it's deployed as build output.
...
- If you're using `@next/mdx` you have to put the files in specific directories, non of which are `public`
...
Now since we're using bundler APIs to include the files they will get includes in the bundle that `webpack` generates.
...
We could configure webpack somehow to do something special for `.mdx` files in this case, but that would be a lot of work.
...
```js title="app/lib/my-custom-loader.js"
